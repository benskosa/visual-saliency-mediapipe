...
def rx_pv(host: str, control_port: int, stream_port: int, chunk: int=hl2ss.ChunkSize.PERSONAL_VIDEO, mode: int=hl2ss.StreamMode.MODE_1,
    ...
    return hl2ss.rx_decoded_pv(host, control_port, stream_port, chunk, mode, width, height, framerate, divisor, profile, level, bitrate, options, decoded_format, send_back, recv_callback)\
          if (decoded_format) else hl2ss.rx_pv(host, control_port, stream_port, chunk, mode, width, height, framerate, divisor, profile, level, bitrate, options, send_back, recv_callback)
...
def ipc_umq(host, port, stream_port=None):
    return hl2ss.ipc_umq(host, port, stream_port)
