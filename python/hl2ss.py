...
# IPC UDP Ports
class IPCUDPPort:
    SPATIAL_MAPPING      = 5001
    SCENE_UNDERSTANDING  = 5002
    VOICE_INPUT          = 5003
    UNITY_MESSAGE_QUEUE  = 5004
    GUEST_MESSAGE_QUEUE  = 5005
...
class _packet:
    def __init__(self, timestamp, payload, pose, senttime = None):
...
class _unpacker:
        ...
        while (True):
            if (self._state == 0):
                if (length >= 12):
                    ...
                    self._last_trunk = bytes(self._buffer[:self._size])
    ...
    def get_last_trunk(self) -> Union[None, bytes]:
        return self._last_trunk
    

class _unpacker_with_senttime:
    def reset(self, mode):
        self._mode = mode
        self._state = 0
        self._buffer = bytearray()
    ...
    def unpack(self):        
        ...
        while (True):
            if (self._state == 0):
                if (length >= 20):
                    header = struct.unpack('<QQI', self._buffer[:20])
                    self._timestamp = header[0]
                    self._senttime = header[1]
                    self._size = 20 + header[2]
                    if (self._mode == StreamMode.MODE_1):
                        self._size += 64
    ...
    def get(self):
        return _packet(self._timestamp, self._payload, self._pose, self._senttime)
...
class _gatherer_udp:
    def open(self, host: str, control_port: int, stream_port: int, chunk_size: int, mode, recv_callback: Callable[[bytes], None] = None):
        ...
        self._unpacker = _unpacker_with_senttime()
        self._chunk_size = chunk_size
        self._unpacker.reset(mode)
        ...
        self.generic_udp = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        
    def sendall(self, data):
        self._client.sendall(data)

    def sendback(self, data: bytes):
        # send back to the stream port
        self.generic_udp.sendto(data, (self.host, self.stream_port))
...
def stop_subsystem_pv(host, port):
    ...
    c.sendall(_create_configuration_for_mode(_PVCNT.STOP | _PVCNT.MODE_3))
    c.close()
...
class rx_pv(_context_manager):
    def __init__(self, host: str, control_port: int, stream_port: int, chunk: int, mode: int, width: int, height: int, framerate: int,
                 divisor, profile, level, bitrate, options, send_back: bool = False, recv_callback: Callable[[bytes], None] = None):
        self.host = host
        self.control_port = control_port
        self.stream_port = stream_port
        self.chunk = chunk
        ...
        self.last_sendback_time = 0
        self.send_back_interval = 0.15 # 150ms
        self.recv_callback = recv_callback

    def open(self):
        # _gatherer_udp
        self._client: _gatherer_udp = _connect_client_pv(self.host, self.control_port, self.stream_port, self.chunk, self.mode, self.width, self.height, self.framerate, self.divisor, self.profile, self.level, self.bitrate, self.options, self.recv_callback)

    def get_next_packet(self) -> _packet:
        packet = self._client.get_next_packet()
        if (self.send_back):
            now = time.time()
            if (now - self.last_sendback_time > self.send_back_interval):
                trunk = self._client.get_last_trunk()
                self._client.sendback(trunk)
                self.last_sendback_time = now

        return packet

    def close(self):
        # first send a close command
        self._client.sendall(_create_configuration_for_mode(_PVCNT.STOP))
        self._client.close()
...
class _decode_rm_vlc:
    ...
    def decode(self, payload):
        try:
            ...
            print('Error decoding RM VLC frame: {}'.format(e))
...
def unpack_pv(payload):
    metadata_len = 84
    image    = payload[0:-metadata_len]
    metadata = payload[-metadata_len:]
...
class rx_decoded_pv(rx_pv):
    def __init__(self, host: str, control_port: int, stream_port: int, chunk: int, mode: int, width: int, height: int, framerate: int,
                 divisor, profile, level, bitrate, options, format, send_back: bool = False, recv_callback=None):
        super().__init__(host, control_port, stream_port, chunk, mode, width, height, framerate, divisor, profile,
                         level, bitrate, options, send_back, recv_callback)
        self.format = format
...
class umq_command_buffer:
    ...
    PROTOCOL_HEARTBEAT = 0
    PROTOCOL_EXIT = 1
    
    @staticmethod
    def protocol_package(command):
        buffer = umq_command_buffer()
        buffer.add(command, b'')
        return buffer
    
HEARTBEAT_PACKAGE = umq_command_buffer.protocol_package(umq_command_buffer.PROTOCOL_HEARTBEAT)
EXIT_PACKAGE = umq_command_buffer.protocol_package(umq_command_buffer.PROTOCOL_EXIT)


class ipc_umq(_context_manager):
    def __init__(self, host, port, stream_port=None):
        self.host = host
        self.port = port
        self.stream_port = stream_port
    ...
    def open(self):
        ...
        self.generic_udp_client = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)

        # send the stream port
        # in the case of 
        if self.stream_port is not None:
            self._client.sendall(_create_configuration_for_udp_port(self.stream_port))
            self.stream_port_sent = True

    def push(self, buffer: umq_command_buffer):
        ...
        try:
            ...
            print(f"UMQ error: {e}, reconnecting")
            self.connected = False
            self._client.close()
            self.connect()
    ...
    def pull(self, buffer: umq_command_buffer):
        if not self.connected:
            return None
