...
from hl2ss import _context_manager, _packet
...
class _source(mp.Process):
    def __init__(self, receiver: _context_manager, event_stop: Event, source_wires, interconnect_wires):
        super().__init__()
        self._source: _context_manager = receiver
        self._event_stop: Event = event_stop
...
def _create_source(receiver: _context_manager, source_wires: _net_source, interconnect_wires: _net_interconnect):
...
class _sink:
    def __init__(self, sink_wires, interconnect_wires):
        self._sink_din: mp.Queue = sink_wires.sink_din
        self._sink_dout: mp.Queue = sink_wires.sink_dout
    ...
    def get_most_recent_frame(self) -> Tuple[int, _packet]:
...
class consumer:
    ...
    def create_sink(self, producer: producer, port: int, manager: SyncManager, semaphore):
